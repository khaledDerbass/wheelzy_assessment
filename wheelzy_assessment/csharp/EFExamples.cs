...
public class ExampleQueries
{
    ...
    public async Task<CarSummaryDTO> GetCarSummary(int carId)
    {
        var q =
            ...
            from cq in db.Set<CarQuote>().Where(x => x.CarId == c.CarId && x.IsCurrent).DefaultIfEmpty()
            from cs in db.Set<CarStatus>().Where(x => x.CarId == c.CarId && x.IsCurrent).DefaultIfEmpty()
